...
public class LocationFilterGoogleMapsElevation implements LocationFilterElevation, AsyncElevationRequestCallee {
    ...
    private void makeElevationRequest(double latitude, double longitude) {
        try {
            ...
            URL url = new URL("https://maps.googleapis.com/maps/api/elevation/json?locations=" + parameters + "&key=" + API_KEY);
            AsyncElevationRequest elevationRequest = new AsyncElevationRequest(this);
            elevationRequest.execute(url);
        }
        catch (MalformedURLException e) {
            Log.e("LocationFilterGoogleMapsElevation", e.getMessage());
        }
    }
